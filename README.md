...
- **横竖屏自动切换**：Android 原生播放器通过临时 `<video>` 预加载元数据判断视频方向，竖屏短视频自动切竖屏；播放控件保留手动横竖屏切换按钮。
- **图片协议修复**：新增 `VideoSource.forceHttpsImage` 按源配置开关；`http://` 海报自动替换为 `https://`；图片加载失败显示统一占位。
- **图片列表性能优化**：自研 `VirtualGrid` 虚拟滚动组件（基于 `IntersectionObserver`），`VodList` 离屏卡片渲染轻量占位，减少重 DOM 与并发图片请求。
- **原生播放器增强**：`initPlayer` 与事件监听加 `try/catch` 与错误提示；引入 `@capacitor-community/keep-awake` 播放页防息屏；保留画面比例切换 UI（实际比例受 `@capgo` 插件 API 限制）。
- **无分类视频源兼容**：首页无 `class` 分类字段时隐藏分类栏，使用默认 `categoryId = 0` 直接加载全部视频列表并支持分页。
