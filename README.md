...
- **横竖屏自动切换**：Android 原生播放器根据视频比例自动识别竖屏短视频并切换，同时保留手动切换按钮。
- **图片协议修复**：针对部分源返回 `http://` 海报但实际需要 `https://` 的问题，提供按源配置的「图片强制 HTTPS」开关。
- **图片列表性能优化**：长列表页面引入虚拟滚动，降低大量图片 DOM 渲染压力；统一图片加载失败占位。
- **原生播放器增强**：稳定化错误处理、画面比例（适应/填充）切换、播放时防止屏幕自动熄灭。
